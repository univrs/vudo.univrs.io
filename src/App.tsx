  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-primary)] flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-[var(--text-muted)] text-sm tracking-widest uppercase">Summoning...</p>
      </div>
    </div>
  );
}
...
function VeveDivider() {
  return (
    <div className="flex items-center justify-center gap-4 md:gap-8 my-12 md:my-16 px-4">
      ...
      <VevePattern type="mitan" size={32} strokeWidth={1} className="md:w-10 md:h-10" />
    ...
    <section className="relative py-16 md:py-32 px-4 md:px-6">
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-[var(--text-muted)] mt-2">VUDO speaks both.</p>
        </motion.div>
          ...
          {vocabulary.map((item, i) => (
            ...
            >
              <div className="mb-3 md:mb-4 opacity-60 group-hover:opacity-100 transition-opacity">
                <VevePattern type={item.type} size={48} color="#8b5cf6" className="mx-auto md:w-[60px] md:h-[60px]" delay={i * 0.2} />
              </div>
              <h3 className="text-base md:text-lg text-[var(--text-primary)] mb-1">{item.term}</h3>
              <p className="text-xs md:text-sm text-[var(--text-muted)]">{item.meaning}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
function ManifestoSection() {
  return (
    <section id="imaginarium" className="relative py-16 md:py-32 px-4 md:px-6 overflow-hidden">
      ...
      <motion.div 
        ...
        viewport={{ once: true }}
      >
        <VevePattern type="mitan" size={60} className="mx-auto mb-6 md:mb-8 md:w-20 md:h-20" />
        ...
        <div className="space-y-4 md:space-y-6 text-base md:text-lg text-[var(--text-secondary)] leading-relaxed px-4">
          ...
          <p>
            ...
            It becomes <em className="text-[var(--text-primary)]">consciousness exploring itself</em>.
          </p>
          <p className="text-[var(--text-muted)] text-sm md:text-base">The Imaginarium is not a product. It is a garden.</p>
        </div>
  ...
  return (
    <section className="relative py-16 md:py-32 px-4 md:px-6">
      <div className="max-w-4xl mx-auto text-center">
        <h2 className="text-xs md:text-sm tracking-[0.2em] md:tracking-[0.3em] uppercase text-[#fbbf24]/80 mb-6 md:mb-8">The Summons</h2>
        
        <p className="text-xl md:text-2xl text-[var(--text-secondary)] mb-8 md:mb-12 px-4">
          The Imaginarium is not built. It is grown.<br />
          <span className="text-[var(--text-muted)]">And it begins with you.</span>
        </p>
        ...
        <div className="grid md:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12">
          <div className="p-5 md:p-6 bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-sm hover:border-[#00ff88]/30 transition-colors">
            ...
            <p className="text-xs md:text-sm text-[var(--text-muted)] mb-4">Clone the repo. Run the tests.<br />Write your first Gene.</p>
            <a href="https://github.com/univrs/metadol" className="text-xs md:text-sm text-[#00ff88] hover:text-[#00ff88]/80">
              Start Building →
            </a>
          </div>
          ...
          <div className="p-5 md:p-6 bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-sm hover:border-[#8b5cf6]/30 transition-colors">
            ...
            <p className="text-xs md:text-sm text-[var(--text-muted)] mb-4">Imagine a Spirit you would summon.<br />Design an experience.</p>
            <a href="#waitlist" className="text-xs md:text-sm text-[#8b5cf6] hover:text-[#8b5cf6]/80">
              Join the Waitlist →
            </a>
          </div>
          ...
          <div className="p-5 md:p-6 bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-sm hover:border-[#fbbf24]/30 transition-colors">
            ...
            <p className="text-xs md:text-sm text-[var(--text-muted)] mb-4">Read the ontology foundations.<br />Critique the architecture.</p>
            <a href="https://learn.univrs.io" className="text-xs md:text-sm text-[#fbbf24] hover:text-[#fbbf24]/80">
              Explore the Theory →
            </a>
          </div>
        </div>
        
        <VevePattern type="mitan" size={48} className="mx-auto mb-4 md:mb-6 md:w-[60px] md:h-[60px]" delay={0.5} />
        <p className="text-[var(--text-muted)] text-xs md:text-sm italic">The Mitan awaits. Enter the Imaginarium.</p>
      </div>
    </section>
  );
}
    ...
    <footer className="relative py-8 md:py-12 px-4 md:px-6 border-t border-[var(--border-color)]">
      <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
        <div className="flex items-center gap-3">
          <VevePattern type="mitan" size={20} animated={false} strokeWidth={1} className="md:w-6 md:h-6" />
          <span className="text-[var(--text-muted)] text-sm">VUDO</span>
          <span className="text-[var(--text-muted)]/50">|</span>
          <span className="text-[var(--text-muted)] text-sm">Univrs.io</span>
        </div>
        
        <div className="flex items-center gap-6 text-sm text-[var(--text-muted)]">
          <a href="https://github.com/univrs" className="hover:text-[var(--text-secondary)]">GitHub</a>
          <a href="https://learn.univrs.io" className="hover:text-[var(--text-secondary)]">Docs</a>
        </div>
        
        <p className="text-xs text-[var(--text-muted)]">© 2025 Univrs. Open Source.</p>
      </div>
    </footer>
  );
}
