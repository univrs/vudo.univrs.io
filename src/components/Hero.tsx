  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      >
        <div className="flex justify-center items-center gap-4 md:gap-8 mb-6">
          {title.split('').map((letter, i) => (
            <motion.span
              ...
              style={{ fontFamily: 'Space Grotesk, sans-serif', textShadow: '0 0 60px rgba(0, 255, 136, 0.3)' }}
            >
              {letter}
            </motion.span>
          ))}
        ...
        <motion.h2 variants={fadeUpVariants} className="text-xl md:text-2xl lg:text-3xl text-white/90 mb-4 font-light">
          Systems that know what they are
        </motion.h2>
        
        <motion.p variants={fadeUpVariants} className="text-base md:text-lg text-white/50 mb-12 max-w-xl mx-auto">
        ...
        <motion.div variants={fadeUpVariants} className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          >
            <span className="relative z-10 tracking-wider uppercase text-sm">Enter the Imaginarium</span>
          </a>
