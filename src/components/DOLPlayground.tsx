...
function highlightDOL(code: string): string {
  // Order matters - more specific patterns first
  const patterns: [RegExp, string][] = [
    // Comments
    [/(\/\/.*$)/gm, '<span class="text-white/40 italic">$1</span>'],
    // Strings
    [/"([^"]*)"/g, '<span class="text-amber-400">"$1"</span>'],
    // Keywords
    [/\b(gene|trait|system|constraint|evolves|exegesis|function|return|match|if|else|for|while|loop|break|continue|where|in|requires|law)\b/g, '<span class="text-violet-400">$1</span>'],
    // Types
    [/\b(Int8|Int16|Int32|Int64|UInt8|UInt16|UInt32|UInt64|Float32|Float64|Bool|String|Void|Self)\b/g, '<span class="text-emerald-400">$1</span>'],
    // Operators
    [/(\|>|>>|@|:=|<\||->|=>)/g, '<span class="text-cyan-400">$1</span>'],
    // Numbers
    [/\b(\d+)\b/g, '<span class="text-pink-400">$1</span>'],
    // Type parameters
    [/<([A-Z][a-zA-Z0-9]*)>/g, '&lt;<span class="text-emerald-400">$1</span>&gt;'],
    // Identifiers after keywords
    [/\b(gene|trait|function|constraint|law)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g, '<span class="text-violet-400">$1</span> <span class="text-white">$2</span>'],
  ];
...
export function DOLPlayground() {
  ...
  const highlightedCode = useMemo(() => highlightDOL(example.code), [example.code]);
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-white/5 border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-3 text-xs text-white/40 font-mono">
                    {example.name.toLowerCase()}.dol
                  </span>
                </div>
                <span className="text-xs text-[#00ff88]/60 font-mono">DOL 2.0</span>
              </div>
            ...
            <div className="bg-[#0d0d14] border border-white/10 rounded-sm overflow-hidden h-full">
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-white/5 border-b border-white/10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/40 font-mono">
                    {isCompiling ? 'compiling...' : showOutput ? 'success' : ''}
                  </span>
                </div>
              </div>
                  ...
                  {isCompiling ? (
                    ...
                    >
                      <div className="text-center">
                        ...
                        />
                        <p className="text-white/40 text-sm">Compiling to MLIR...</p>
                      </div>
                    </motion.div>
                  ) : (
                    <motion.pre
